...
class Fit:
    ...
    presentations of draws are available via the `to_frame` and `to_panel`
    methods.
    ...
    def __init__(self, stan_outputs: Sequence[Sequence[Mapping[str, Any]]], num_chains: int,
                 param_names: Sequence[str], constrained_param_names: Sequence[str],
                 dims: Sequence[Sequence[int]], num_warmup: int, num_samples: int,
                 num_thin: int, save_warmup: bool) -> None:
        self.stan_outputs = stan_outputs
        self.num_chains = num_chains
        assert self.num_chains == len(self.stan_outputs)
        self.param_names, self.dims, self.constrained_param_names = param_names, dims, constrained_param_names
        self.num_warmup, self.num_samples = num_warmup, num_samples
        self.num_thin, self.save_warmup = num_thin, save_warmup

        num_params = sum(np.product(dims_ or 1) for dims_ in dims)  # if dims == [] then it is a scalar
        ...
        for chain_index, stan_output in zip(range(self.num_chains), self.stan_outputs):
            draw_index = 0
            for output_index, entry in enumerate(stan_output):
    ...
    def to_frame(self):
        ...
        df = pd.DataFrame(self._draws.reshape(-1, len(constrained_param_names)),
                          columns=constrained_param_names)
        df.index.name, df.columns.name = 'draws', 'parameters'
        return df

    def to_panel(self):
        """Return view of draws as a pandas Panel.

        If pandas is not installed, a `RuntimeError` will be raised.

        Returns:
            pandas.Panel: Panel with shape
                (`num_chains`, `num_draws`, `num_flat_params`).

        """
        try:
            import pandas as pd
        except ImportError as ex:
            raise RuntimeError('The `to_panel` method requires the Python package `pandas`.')
        constrained_param_names = self.constrained_param_names
        panel = pd.Panel(self._draws, minor_axis=constrained_param_names)
        panel.items.name = 'chains'
        panel.major_axis.name, panel.minor_axis.name = 'draws', 'parameters'
        return panel
